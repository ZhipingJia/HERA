...
class FasterRCNN(nn.Module):
    ...
    def forward(self, x, scale=1.):
        ...
        * :math:`L` is the number of classes excluding the background.
        ...
        h = self.extractor(x)
        # if len(h)==2:
            # print('h[0].max:',h[0].max())
            # print('scale multiplied:',h[1])
            # print('h[0].max()*h[1]',h[0].max()*h[1])
            # h = h[0]*h[1]
            # print('h.max:',h.max())
            #print('last layer scale:',h[1])
            # h = h[0]*0.25
            # h = h[0]
    ...
    @nograd
    def predict(self, imgs, sizes=None, visualize=False, min_size=600, max_size=1000):
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            if isinstance(roi_scores,tuple):
                roi_scores = roi_scores[0]*roi_scores[1]
            
            roi_score = roi_scores.data

            if isinstance(roi_cls_loc,tuple):
                roi_cls_loc = roi_cls_loc[0]*roi_cls_loc[1]
