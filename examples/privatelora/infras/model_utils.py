...
EMBEDDING_LMHEADLORA_TRAINING = False
...
class Llama2BaseModelTokenizerHandler(BaseModelTokenizerHandler):
    # Base-model location: set PRIVATE_LLM_BASE_MODEL to a local llama-2-7b-hf
    # snapshot or keep the default Hugging Face Hub id (requires accepting the
    # Llama 2 license on the Hub).  The base model is not redistributed here.
    model_paths = {
        "7b": os.environ.get("PRIVATE_LLM_BASE_MODEL", "meta-llama/Llama-2-7b-hf"),
    }
...
class PrivateLlama(Llama2BaseModelTokenizerHandler):
    def model_post_init(self, model):
        ...
        if EMBEDDING_LMHEADLORA_TRAINING:
            for n, p in model.named_parameters():
                if "lora_mobile" not in n and "lora_lm_head" not in n and "embed_tokens" not in n:
